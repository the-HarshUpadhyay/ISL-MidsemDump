...
2) Design a Python-based experiment to analyze the performance of MD5, SHA-1, and SHA-256
hashing techniques in terms of computation time and collision resistance. Generate a dataset
of random strings ranging from 50 to 100 strings, compute the hash values using each
hashing technique, and measure the time taken for hash computation. Implement collision
detection algorithms to identify any collisions within the hashed dataset.

Additional Exercise
3) Write server and client scripts where the client sends a message in multiple parts to
...
the message by comparing the received hash with the locally computed hash of the original
message.
...
================================================================================
ADDITIONAL EXERCISE 1 — MULTIPART MESSAGE HASHING (SERVER)
Write server and client scripts where the client sends a message in multiple parts to the
server, the server reassembles the message, computes the hash of the reassembled message,
and sends this hash back to the client. The client then verifies the integrity of the message
by comparing the received hash with the locally computed hash of the original message.

This file implements the SERVER portion of the additional exercise.
"""
...
with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as server_socket:
    ...
    print(f"Multipart server listening on {HOST}:{PORT}")
    ...
    with connection:
        ...
        number_of_parts = int(
            receive_exact(connection, 4).decode("utf-8")
        )

        received_parts = []

        for _ in range(number_of_parts):
            part_length = struct.unpack(
                "!I", receive_exact(connection, 4)
            )[0]

            part = receive_exact(connection, part_length)
            received_parts.append(part)

        complete_message = b"".join(received_parts)
        message = complete_message.decode("utf-8")

        result_hash = user_defined_hash(message)

        print("Reassembled message:", message)
        print("Computed hash:", result_hash)

        connection.sendall(str(result_hash).encode("utf-8"))
